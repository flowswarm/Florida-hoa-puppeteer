  ...
  try {
    ...
    await page.evaluate(() => {
      const link = Array.from(document.querySelectorAll('a'))
        .find(a => a.innerText.includes('Instant Public Records'));
      if (link) link.click();
    });
    await page.waitForNavigation();
    ...
    await page.evaluate(() => {
      const link = Array.from(document.querySelectorAll('a'))
        .find(a => a.innerText.includes('Community Association Managers'));
      if (link) link.click();
    });
    await page.waitForNavigation();
    ...
    // Find and go to CSV link
    const csvUrl = await page.evaluate(() => {
      const link = Array.from(document.querySelectorAll('a'))
        .find(a => a.href.endsWith('.csv'));
